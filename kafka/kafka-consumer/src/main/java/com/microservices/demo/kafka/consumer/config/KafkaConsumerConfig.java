 ...
 * config-client-kafka_to_elastic this class makes the configuration values into use able kafka.core/config Spring
 * Beans.
 *
 * ConsumerFactory is configured via KafkaConfigData and KafkaConsumerConfigData and then itself used to configure
 ...
 * @param <V> As Avro is being used SpecificRecordBase is an Avro abstract class extended by Avro Generated
 *            Models that we make templates for.
 */
@EnableKafka
@Configuration
...
public KafkaListenerContainerFactory<ConcurrentMessageListenerContainer<K, V>> kafkaListenerContainerFactory() {
    ConcurrentKafkaListenerContainerFactory<K, V> concurrentMLCFactory =
    new ConcurrentKafkaListenerContainerFactory<>();

    concurrentMLCFactory.setConsumerFactory(consumerFactory());
    concurrentMLCFactory.setBatchListener(kafkaConsumerConfigData.getBatchListener());
    concurrentMLCFactory.setConcurrency(kafkaConsumerConfigData.getConcurrencyLevel());
    concurrentMLCFactory.setAutoStartup(kafkaConsumerConfigData.getAutoStartup());
    concurrentMLCFactory.getContainerProperties().setPollTimeout(kafkaConsumerConfigData.getPollTimeoutMs());

    return concurrentMLCFactory;
}

...
public class MockKafkaStreamRunner implements StreamRunner {
  ...
  /**
   * INFO: Simulated Twitter Stream running in a separate thread so as not to block the main thread,
   * the lambda within the submit method of Executors.newSingleThreadExecutor actually implements
  ...
  private void sleep(long sleepTimeMs) {
    try {
      ...
      throw new TwitterToKafkaServiceException(
          "Error while sleeping between status' being created in " + "MockStreamRunner", e);
    }
  }
...
private String formatStringAsJsonWithParams(String[] params) {
...
private String getRandomTweetContent(String[] keywords, int minTweetLength, int maxTweetLength) {
  ...
  LOG.info("Tweet length is {} correct? now?", tweetLength);
